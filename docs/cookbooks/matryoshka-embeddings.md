...
```lua
{{#include examples/matryoshka_embeddings/transform.lua}}
```
...
Ensure the encoderfile CLI is installed and available in your `PATH`. For instructions on how to install the encoderfile CLI, check out our [Getting Started](../getting-started.md#encoderfile-cli-tool) guide.
