...
def validate_embedding_model(model: ModelProto):
    ...
    lhs = next(filter(lambda m: m.name == "last_hidden_state", model.graph.output))

    # make sure returns matrix of rank 3
    if len(lhs.type.tensor_type.shape.dim) != 3:
        raise ValueError(
            "Model must return tensor of shape [batch_size, sequence_length, hidden_state_size]"
        )
...
def validate_sequence_classification_model(model: ModelProto):
    ...
    # get logits
    logits = next(filter(lambda m: m.name == "logits", model.graph.output))
...
def validate_token_classification_model(model: ModelProto):
    ...
    if len(logits.type.tensor_type.shape.dim) != 3:
        raise ValueError(
            "Model must return tensor of shape [batch_size, n_tokens, n_labels]"
        )
