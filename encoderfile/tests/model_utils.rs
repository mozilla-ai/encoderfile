use encoderfile::{common::ModelType, model::config::ModelConfig, state::AppState};
...
fn get_tokenizer(dir: &str, config: &Arc<ModelConfig>) -> tokenizers::Tokenizer {
    ...
    encoderfile::model::tokenizer::get_tokenizer_from_string(tokenizer_str.as_str(), &config)
}
