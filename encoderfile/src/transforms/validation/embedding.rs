...
pub fn validate_transform(transform: Transform) -> Result<()> {
    ...
    let dummy_hidden_states = utils::random_tensor(&[batch_size, seq_len, hidden_dim], (-1.0, 1.0))?;
    ...
    if res.ndim() != 3 {
        bail!("Transform must return tensor of rank 3. Got tensor of shape {:?}", res.shape());
    }
