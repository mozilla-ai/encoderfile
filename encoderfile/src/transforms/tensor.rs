    ...
    fn from_lua(value: LuaValue, _lua: &Lua) -> Result<Self, LuaError> {
        match value {
            ...
            LuaValue::UserData(data) => {
                data.borrow::<Tensor>().map(|i| i.to_owned())
            },
            _ => Err(LuaError::external(format!("Unknown type: {}", value.type_name()).as_str()))
        }
    }
}
...
impl Tensor {
    ...
    fn lp_norm(&self, p: f32) -> Result<f32, LuaError> {
        ...
        Ok(self.0.iter().map(|v| v.abs().powf(p)).sum::<f32>().powf(1.0 / p))
    }
...
mod tests {
    ...
    fn test_from_lua_create_table() {
        ...
        let tbl: LuaTable = get_function(&lua, "CreateGoodTable")
            .call(())
            .unwrap();
    ...
    fn test_from_lua_ragged() {
        ...
        let tbl: LuaTable = get_function(&lua, "CreateRaggedTable")
            .call(())
            .unwrap();
    ...
    fn test_from_lua_bad_type() {
        ...
        let tbl: LuaTable = get_function(&lua, "CreateStringTable")
            .call(())
            .unwrap();
    ...
    #[test]
    fn test_softmax() {
        let arr: ArrayD<f32> = ndarray::array![
            [1.0, 2.0],
            [4.0, 5.0],
            ].into_dyn();
    ...
    fn test_l1_norm() {
        ...
        assert!((norm - expected).abs() < 1e-6, "Expected {}, got {}", expected, norm);
    }
